...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_L2=affine_forward(out_L1,W2,b2)       
        ...
        #Backprop 2nd layer
        dx1, dW2, db2 =affine_backward(dscores, cache_L2)
        dW2 += self.reg * W2
        
        #Backprop 1st layer
        dx, dW1, db1=affine_relu_backward(dx1,cache_L1)
        dW1 +=self.reg *W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for k, v in self.params.items(): #k=key, v=value for dict
            #print(k)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.normalization=='batchnorm':
                layer_output[i], layer_cache[i] = affine_bn_relu_forward(layer_output[i-1],
        ...
        scores, cache_scores = affine_forward(layer_output[self.num_layers - 1],
                                                  self.params[WLast],
                                                  self.params[bLast])
        ...
        for i in reversed(range(1, self.num_layers)):
            ...
            if self.normalization=='batchnorm':
                dx[i], grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)],grads['beta'+str(i)] = affine_bn_relu_backward(dx[i+1], layer_cache[i])
